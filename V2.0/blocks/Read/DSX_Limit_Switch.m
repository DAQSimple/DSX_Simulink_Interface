...
function Outputs(block)
    ...
    [DSXval,DSXsign,DSXstruct] = Serial_Receive_callback('checkBuffer',limit_init); % use initial ping
    
    if ~isempty(DSXval) % if there was succesfully a CMD21 ping
        if DSXstruct.loc == num2str(block.DialogPrm(1).Data) % is it for this block?           
            
            if DSXstruct.sign == '0' 
                % not crucial stop, execute commands
                if DSXstruct.ret == '1' % write 1  
                    block.OutputPort(1).Data = 1;
                else DSXstruct.ret == '0' % write 0
                    block.OutputPort(1).Data = 0;
                end
                
            else DSXstruct.sign == '1'
